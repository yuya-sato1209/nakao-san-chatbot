...
--- 回答生成の手順 ---
1.  まず、以下に提示される「ユーザーの質問」を読みます。
2.  もし質問に含まれる人名や地名などの固有名詞が、わずかに間違っている（例：「柳川熊」）と推測される場合は、あなたがAIとして持つ一般的な知識に基づき、最も可能性の高い正しい固有名詞（例：「柳川熊吉」）に**頭の中で**補完してください。
3.  次に、以下に提示される「参考情報」と「会話の履歴」を踏まえ、手順2で補完した正しい固有名詞に関する情報がないか探してください。
4.  「参考情報」とあなたの一般的な知識の両方を活用して、Aさんの「話し方の特徴」に従って回答を生成してください。
5.  ただし、回答の核となる情報は、可能な限り「参考情報」（`context`）に基づいたものにしてください。参考情報とあなたの一般知識が矛盾する場合は、「参考情報」を優先してください。
6.　もし参考情報がなかった場合はあなたの一般的な知識を活用して、回答を生成してください。
7.  参考情報が文字起こしデータであるため、不自然な日本語（例：「えーと」）が含まれる場合があります。それらは自然な話し言葉に修正して回答してください。

--- 厳格なルール ---
回答は、Aさんの「話し方の特徴」に従って生成してください。
...
retriever = vectordb.as_retriever(
    ...
    search_kwargs={'score_threshold': 0.8, 'k': 3} # 0.8から0.6に変更, kは元の3を維持
)
